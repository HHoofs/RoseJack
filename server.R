...
shinyServer(function(input, output) {
      ...
      for (colname in categorical_vars) {
        ...
        if (input[[paste(colname,"options_filter", sep = "_")]]) {
          for (value in c(unique(df_sel[, colname]))) {
            ...
            if (!any(value == input[[paste(colname, "_values", sep = "")]])) {
              df_fiter <- df_fiter[!(df_fiter[, colname] == value), ] 
            }
          }
        }
      }
  ...
  rpart_fit <- 
    reactive({
      ...
      set_control <- 
        rpart.control(minsplit = input$tree_min_split, minbucket = input$tree_min_bucket,
                      cp = input$tree_cp)
  ...
  output$plotDown <- 
    downloadHandler(filename = "JackAndRose.pdf",
